...
namespace Agentic.Runtime.Core;
...
public static class LlamaRuntimeInstaller
{
    ...
    /// Windows: <c>%LOCALAPPDATA%\Agentic\llama-runtime</c>
    /// Linux/macOS: <c>~/.local/share/Agentic/llama-runtime</c>
    /// </summary>
    ...
    public static string? FindInstalled(LlamaBackend backend, string? releaseTag = null, string? installRoot = null)
    {
        ...
        string prefix = BackendPrefix(backend);
        ...
        CancellationToken ct = default)
    {
        ...
        http.DefaultRequestHeaders.Add("User-Agent", "Agentic-Runtime/1.0");
        ...
        var asset = PickAsset(release.Assets, backend, cudaVersion, isWindows)
        ...
        try
        {
            ...
            try
            {
                ...
                string installDir = Path.Combine(root, $"{BackendPrefix(backend)}-{tag}");
        ...
        switch (backend)
        {
            case LlamaBackend.Cuda:
            {
                var cudaAssets = assets
                    .Where(a => Has(a, os) && Has(a, "cuda") && !Has(a, "vulkan") && IsArchive(a))
                    .ToList();

                if (cudaVersion is not null)
                    return cudaAssets.FirstOrDefault(a => Has(a, cudaVersion));
            ...
            case LlamaBackend.Vulkan:
                return assets.FirstOrDefault(a => Has(a, os) && Has(a, "vulkan") && IsArchive(a));

            default: // Cpu — prefer avx2, then avx, then noavx, then any non-GPU asset
                return
                    assets.FirstOrDefault(a => Has(a, os) && Has(a, "avx2") && !Has(a, "cuda") && !Has(a, "vulkan") && IsArchive(a)) ??
                    assets.FirstOrDefault(a => Has(a, os) && Has(a, "avx") && !Has(a, "cuda") && !Has(a, "vulkan") && IsArchive(a)) ??
                    assets.FirstOrDefault(a => Has(a, os) && !Has(a, "cuda") && !Has(a, "vulkan") && !Has(a, "opencl") && !Has(a, "sycl") && IsArchive(a));
        }
    }
    ...
    private static string BackendPrefix(LlamaBackend backend) => backend switch
    {
