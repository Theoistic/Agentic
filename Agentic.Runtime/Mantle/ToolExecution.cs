...
public sealed record McpServerProcessOptions(
    ...
    string ClientName = "Agentic.Runtime",
...
public sealed class DefaultToolExecutionEngine : IToolExecutionEngine
{
    ...
    public async Task<string> ExecuteAsync(ToolExecutionRequest request, CancellationToken ct = default)
    {
        ...
        using var activity = RuntimeTelemetry.StartActivity("runtime.tool.execute");
        RuntimeTelemetry.ToolExecutions.Add(1, new KeyValuePair<string, object?>("agentic.runtime.tool.name", request.Call.Name));
        var sw = Stopwatch.StartNew();
        try
        {
            activity?.SetTag("agentic.runtime.tool.name", request.Call.Name);
            activity?.SetTag("agentic.runtime.tool.is_remote", request.Tool.IsRemote);
        ...
        {
            RuntimeTelemetry.ToolExecutionErrors.Add(1, new KeyValuePair<string, object?>("agentic.runtime.tool.name", request.Call.Name));
        ...
        finally
        {
            RuntimeTelemetry.ToolExecutionDuration.Record(sw.Elapsed.TotalMilliseconds,
                new KeyValuePair<string, object?>("agentic.runtime.tool.name", request.Call.Name));
        }
    }
}
...
public sealed class McpRemoteToolExecutor : IRemoteToolExecutor, IAsyncDisposable, IDisposable
{
    ...
    private static bool TryResolveServerOptions(IReadOnlyDictionary<string, object?> metadata, out McpServerProcessOptions options)
    {
        ...
        options = new McpServerProcessOptions(
            ...
            ClientName: metadata.TryGetValue("clientName", out var cn) && cn?.ToString() is { Length: > 0 } clientName ? clientName : "Agentic.Runtime",
